...
export default function ProductVendorsPage() {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/admin/vendors?type=product`, {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">🛒 Product Sale Vendors</h1>
            <p className="text-gray-500 mt-1">Manage wedding product sellers</p>
          </div>
          <select
            value={filter}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-6 text-white">
            <p className="text-white/80">Total Product Vendors</p>
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
            ...
            <p className="text-2xl font-bold text-teal-600">Rs. 0</p>
          </div>
        </div>
              ...
              <div key={vendor._id} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow">
                <div className="p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p>📧 {vendor.email}</p>
                    <p>📞 {vendor.phone}</p>
                    <p>📦 Products: {vendor.totalProducts || 0}</p>
                    <p>💰 Sales: Rs. {vendor.totalSales?.toLocaleString() || 0}</p>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 flex gap-2">
                    ...
                    {vendor.status === 'pending' && (
                      <>
                        <button className="px-3 py-2 text-sm bg-green-50 text-green-600 rounded-lg hover:bg-green-100">✅</button>
                        <button className="px-3 py-2 text-sm bg-red-50 text-red-600 rounded-lg hover:bg-red-100">❌</button>
                      </>
                    )}
                  </div>
                </div>
