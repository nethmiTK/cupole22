...
export default function AlbumVendorsPage() {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'}/api/admin/vendors?type=album`, {
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-1">Manage wedding album providers</p>
          </div>
          <select
            value={filter}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-xl p-6 text-white">
            <p className="text-white/80">Total Album Vendors</p>
              ...
              <div key={vendor._id} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow">
                <div className="p-6">
                  ...
                  <div className="space-y-2 text-sm text-gray-600">
                    <p>📧 {vendor.email}</p>
                    <p>📞 {vendor.phone}</p>
                    <p>📷 Albums: {vendor.totalAlbums || 0}</p>
                    <p>📅 Joined: {new Date(vendor.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="mt-4 pt-4 border-t border-gray-100 flex gap-2">
                    <button className="flex-1 px-3 py-2 text-sm bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100">
                      View Details
                    </button>
